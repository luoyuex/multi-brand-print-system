...
_DEFAULTS = {
    ...
    "paper_size": "140x241", # 竖版 PDF：140宽 × 241高，与自定义纸「python」(241×140)+纵向一致。
                             # 内容自然往下流（标题在上、商品行向下），可放 ~40 行，不裁切、不换页。
                             # SumatraPDF 用 noscale 按 PDF 页面宽高比决定方向：竖版 → 纵向送，
                             # 与 WPS「python纸+纵向」打对的那份一致。不做任何 CSS/代码旋转。
    "copies": 1,
    "sumatra_path": "",      # 空 = 自动探测
    "print_rotate": 90,      # 渲染竖版 140x241，打印前用 pymupdf 烤成实体横版 241x140 再送打印机。
                             # 打印机驱动选「横向 + python纸」。若方向反 180°，改成 270。
}
