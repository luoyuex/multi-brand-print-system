...
_DEFAULTS = {
    ...
    "paper_size": "241x140", # 一联记账凭证：241宽 × 139.5高（二联纸的半张，一联）。
                             # 241=两针孔之间(走纸宽度)，139.5=一联走纸长度(撕裂线之间)。
                             # 模板内用 CSS 把内容整块旋转 90°：行沿 241mm 长边排，一联可放 ~40 行。
                             # PDF 页面保持 241×139.5 物理尺寸，1:1 送打印机，不再靠 print_rotate 转向。
    "copies": 1,
    "sumatra_path": "",      # 空 = 自动探测
    "print_rotate": 0,       # 不旋转：旋转已在模板 CSS 层面完成，PDF 页面 241x139.5 直接 1:1 送打印机。
}
