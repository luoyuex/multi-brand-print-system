"""通过独立子进程调用 Playwright 把账单小票 HTML 截图成 PNG。
...
送货单已改走 GDI 直接绘制，不再经过本脚本；这里只保留账单小票截图。
"""
...
def main():
    """
    从 stdin 读取 JSON: { html_path, img_path }
    用 Playwright 渲染 HTML，截图成 PNG。
    成功输出 JSON: { ok: true }
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # 小票是固定窄幅（body 420px），只截 body 元素，避免默认 1280 视口
            # 把小票截成一角带大片留白。device_scale_factor=2 更清晰。
